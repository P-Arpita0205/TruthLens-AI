...
export default function PhotoAnalysis() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/analyze/quota?type=photo`)
      .then(r => r.json())
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      setError(analysisError.message || 'Failed to analyze photo');
      setResult(null);
    } finally {
      setAnalyzing(false);
    }
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-medium text-slate-400">
              Upload an image for a premium, in-memory authenticity check with structured AI reasoning and clear confidence output.
            </p>
            {error ? (
              ...
              {file ? (
                <motion.div initial={{ scale: 0.96, opacity: 0.8 }} animate={{ scale: 1, opacity: 1 }} className="relative z-0 flex flex-col items-center">
                  <div className="mb-5 flex h-20 w-20 items-center justify-center rounded-3xl bg-white/10 shadow-lg shadow-cyan-500/20 ring-1 ring-white/10">
                    <ImageIcon className="h-9 w-9 text-cyan-300" />
                  </div>
                  <p className="max-w-sm truncate text-2xl font-bold tracking-tight text-white">{file.name}</p>
                  <p className="mt-2 text-sm font-medium text-cyan-200">
                    {(file.size / (1024 * 1024)).toFixed(2)} MB • Ready for visual analysis
                  </p>
                </motion.div>
              ) : (
                <div className="relative z-0 flex flex-col items-center">
                  ...
                  <p className="text-2xl font-bold tracking-tight text-white">Drop your image here</p>
                  <p className="mt-3 text-sm font-medium text-slate-400">JPG, PNG, WEBP up to 10MB</p>
            ...
            >
              {analyzing ? (
                <>
                  ...
                  <span>Scanning metadata and facial cues...</span>
                </>
              ) : (
              ...
              {analyzing ? (
                ...
                >
                  <div className="mb-8 flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-white">Running Multi-Agent Check</h3>
                      <p className="mt-2 text-sm text-slate-400">Evaluating visual clues, frame consistency, and explanation quality.</p>
                    </div>
                  </div>
                ...
                >
                  <div>
                    ...
                    <p className="mt-3 text-sm font-medium text-slate-400">
                      Start a photo analysis to see the verdict, confidence score, and plain-language explanation.
                    </p>
                  </div>
                </motion.div>
              ) : null}
